...
def bdBuildDrvChain(side,drvType):
	print 'Building %s side %s'%(side, drvType)
	shoulderJnt = pm.ls(side + '_' + armBones[0] + '_bnd', type='joint')[0]
...
def bdScaleChain(side):
	pm.select(cl=True)
	ikAnimCon = pm.ls(side + '_arm_ikfk_ctrl',type='transform')[0]
